...
class Optimize(object):
    ...
    def run_lolbo(self): 
        ...
        while self.lolbo_state.objective.num_calls < self.max_n_oracle_calls:
            ...
            # pass top k through VAE (or top % or random selection of top k)
            self.lolbo_state.pass_top_xs_through() 
            # filter out ones surrogate doesn't think will score high 
            self.lolbo_state.remove_xs_surrogate_deems_bad() 
            # evaluate the rest 
            self.lolbo_state.evaluate_good_xs()
            self.lolbo_state.update_best_found() # record best scoring, etc.
            # back prop VAE + surrogate e2e only one ones that score in top %
            self.lolbo_state.updatee_e2e_on_high_scoring_xs()
            # update surrogate alone on remaining (low scoring included) 
            self.lolbo_state.recenter_low_scoring_xs() 
            self.lolbo_state.surrogate_on_all_xs() 
